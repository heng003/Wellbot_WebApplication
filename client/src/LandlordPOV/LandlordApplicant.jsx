...
function LandlordApplicant() {
    ...
    const toggleRatingSort = () => {
        setSelectedRatingSort(prevSort => prevSort === 'Highest to Lowest' ? 'Lowest to Highest' : 'Highest to Lowest');
    };
    ...
    const renderStatus = (status) => {
        if (status === 'Signed') {
            return (
                <div className='signedStatusData' onClick={handleDownloadSigned} onMouseEnter={() => setIsHovering(true)} onMouseLeave={() => setIsHovering(false)}>
        ...
        if (!selectedProperty || selectedProperty === 'Please Select Your Property') {
            ...
            switch (selectedProperty) {
                ...
                case properties[2]:
                    return (
                        <>
                            <h3 className="notHistory_Text">Sorry, this property hasn't been applied yet, so it <b>doesn't have any applicant's info</b>.</h3>
                        </>
                    );
                default:
                    return (
        ...
        <div className="rental-history">
            ...
            <div className="property-selectors-group">
                ...
                <div className="property-selector" ref={dropdownRatingRef}>
                    ...
                    <div className="custom-select" tabIndex={0} onClick={() => setIsOpenRating(!isOpenRating)} onBlur={() => setIsOpenRating(false)}>
                        <div className="displayed-value">{selectedRatingSort}</div>
                        {isOpenRating && (
                            <div className="custom-options">
                                <div className={`custom-option ${selectedRatingSort === 'Highest to Lowest' ? 'selected' : ''}`} onClick={toggleRatingSort}>Highest to Lowest</div>
                                <div className={`custom-option ${selectedRatingSort === 'Lowest to Highest' ? 'selected' : ''}`} onClick={toggleRatingSort}>Lowest to Highest</div>
                            </div>
                        )}
                    </div>
                </div>
