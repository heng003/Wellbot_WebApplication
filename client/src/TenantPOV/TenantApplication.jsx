  ...
  useEffect(() => {
    async function fetchPropertyInfo() {
      ...
      try {
        ...
        const landlordIds = [
          ...new Set(propertyData.map((data) => data.property.landlordId)),
        ];
        const landlordResponses = await Promise.all(
          landlordIds.map((id) => axios.get(`/api/users/${id}`))
        );
        ...
        const enrichedPropertyData = propertyData.map(
